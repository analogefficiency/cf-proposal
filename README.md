...
10. Perhaps it would be more appropriate, as opposed to creating an endpoint for the short url to hit via the service - that the service create a DNS record that points to the desired url. I'd have to look into this more, but there may be value in deferring the actual redirect to another service and controlling the scope of the service to generating and managing those DNS changes. 
